...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Detect a cell where you click on
    ...
    def load_data(self):
        ...
        for row_numer, row_data in enumerate(result):
            self.table.insertRow(row_numer)
            for column_number, data in enumerate(row_data):
                # print(column_number, data)
                self.table.setItem(row_numer,column_number, QTableWidgetItem(str(data)))
        connection.close()
...
class EditDialog(QDialog):
    ...
    def update_student(self):
        connection = DatabaseConnection().connect()
        # cursor connection we because we want to modify the database
        ...
        # because it is the write to database that why we use commit
        connection.commit()
        cursor.close()
        # Refresh the table
...
class DeleteDialog(QDialog):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Deleted Student Data")
...
class SearchDialog(QDialog):
    ...
    # Search with Start with letter function
    def search_student1(self):
        ...
        items = main_window.table.findItems(
            name, Qt.MatchFlag.MatchStartsWith
        )
        for item in items:
            main_window.table.item(item.row(), 1).setSelected(True)
